...
test("creating a topic space derives the short code and adds a Notizen page", async ({
  page,
}) => {
  await page.goto("/");
  await page
    .getByRole("button", { name: "Themen hinzufügen" })
    .dispatchEvent("click");
  ...
  await expect(
    page.getByRole("button", { name: "Notizen", exact: true }),
  ).toBeVisible();
  await expect(page.getByText("Diese Seite ist leer.")).toBeVisible();

  // Clean up.
  await page
    .getByRole("button", { name: "Release Plan entfernen" })
    .dispatchEvent("click");
  await page.getByRole("button", { name: "Löschen" }).dispatchEvent("click");
  await expect(space).toHaveCount(0);
});

test("deleting a person space keeps foreign tasks and only drops the assignment", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await page
    ...
    .dispatchEvent("click");
  await page
    .getByRole("button", { name: "Personen hinzufügen" })
    .dispatchEvent("click");
  ...
  await expect(
    page.getByRole("button", { name: "Block anlegen" }),
  ).toBeVisible();
  await page
    .getByRole("button", { name: "Block anlegen" })
    .dispatchEvent("click");
  ...
  const hostBlock = page
    .locator("[data-block-id]")
    .filter({ has: page.locator('textarea:has-text("Neuer Meeting")') });
  ...
  await page.keyboard.press("Enter");
  await expect(hostBlock.locator('textarea:has-text("Check")')).toBeVisible();
  ...
  await page
    .getByRole("button", { name: "Kim Lee entfernen" })
    .dispatchEvent("click");
  await expect(
    page.getByRole("dialog", { name: "Löschen bestätigen" }),
  ).toContainText(
  ...
  await page
    .getByRole("button", { name: "Host Raum entfernen" })
    .dispatchEvent("click");
  await page.getByRole("button", { name: "Löschen" }).dispatchEvent("click");
...
test("pages can be created and renamed", async ({ page }) => {
  ...
  await page
    .getByRole("button", { name: "Seite hinzufügen" })
    .dispatchEvent("click");
  await page.getByLabel("Neue Seite").fill("Retrospektive");
  await page.keyboard.press("Enter");
  await expect(
    page.getByRole("button", { name: "Retrospektive", exact: true }),
  ).toBeVisible();
  await expect(page.getByText("Diese Seite ist leer.")).toBeVisible();

  await page
    .getByRole("button", { name: "Retrospektive umbenennen" })
    .dispatchEvent("click");
  await page.getByLabel("Seitentitel umbenennen").fill("Retro");
  await page.keyboard.press("Enter");
  await expect(
    page.getByRole("button", { name: "Retro", exact: true }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Retrospektive", exact: true }),
  ).toHaveCount(0);
  ...
  // Clean up.
  await page
    .getByRole("button", { name: "Seiten Test entfernen" })
    .dispatchEvent("click");
  await page.getByRole("button", { name: "Löschen" }).dispatchEvent("click");
...
test("templates can be added, edited, and deleted", async ({ page }) => {
  ...
  await page
    ...
    .dispatchEvent("click");
  await expect(
    page.getByRole("menuitem", { name: /Templates bearbeiten/ }),
  ).toBeVisible();
  await page
    .getByRole("menuitem", { name: /Templates bearbeiten/ })
    .dispatchEvent("click");
  ...
  await dialog
    .getByRole("button", { name: "Template hinzufügen" })
    .dispatchEvent("click");
  const newCard = dialog
    .locator("[data-template-id]")
    .filter({ has: page.locator("input[value='Neues Template']") });
  await expect(newCard).toHaveCount(1);
  await newCard.getByLabel("Name des Templates").fill("Daily");
  const dailyCard = dialog
    .locator("[data-template-id]")
    .filter({ has: page.locator("input[value='Daily']") });
  await dailyCard.getByLabel("Farbe").selectOption("moss");
  await dailyCard
    .getByLabel("Vorbelegte Zeilen")
    .fill("# Teilnehmer\n# Agenda");
  ...
  await page
    ...
    .dispatchEvent("click");
  await dailyCard
    .getByRole("button", { name: "Daily entfernen" })
    .dispatchEvent("click");
  await page.getByRole("button", { name: "Löschen" }).dispatchEvent("click");
...
test("a block is created from a template with its seed lines", async ({
  page,
}) => {
  await page.goto("/");
  await page
  ...
  await expect(
    block
      .locator("textarea[aria-label='Überschrift']")
      .filter({ hasText: "Teilnehmer" }),
  ).toBeVisible();
  await expect(
    block
      .locator("textarea[aria-label='Überschrift']")
  ...
  // Clean up.
  await page
    .getByRole("button", { name: "Block Test entfernen" })
    .dispatchEvent("click");
  await page.getByRole("button", { name: "Löschen" }).dispatchEvent("click");
  await expect(spaceButton(page, "Block Test")).toHaveCount(0);
});

test("rows and blocks can be deleted, with an in-line block confirmation", async ({
  page,
}) => {
  await page.goto("/");
  await page
  ...
  const headingRow = card
    .locator("[data-item-id]")
    .filter({ has: page.locator('textarea:has-text("Teilnehmer")') });
  await headingRow
    .getByRole("button", { name: "Zeile entfernen" })
    .dispatchEvent("click");
  await expect(
    card
      .locator("textarea[aria-label='Überschrift']")
  ...
  await card
    .getByRole("button", { name: "Block entfernen" })
    .dispatchEvent("click");
  await expect(
    page.getByRole("dialog", { name: "Löschen bestätigen" }),
  ).toContainText("Verweise von außen verlieren ihr Ziel");
  await page.getByRole("button", { name: "Löschen" }).dispatchEvent("click");
  await expect(card).toHaveCount(0);

  // Clean up the space.
  await page
    .getByRole("button", { name: "Del Test entfernen" })
    .dispatchEvent("click");
  await page.getByRole("button", { name: "Löschen" }).dispatchEvent("click");
...
test("a page can be deleted and the selection moves on", async ({ page }) => {
  ...
  await page.keyboard.press("Enter");
  await expect(
    page.getByRole("button", { name: "Doku", exact: true }),
  ).toBeVisible();
  ...
  await page
    .getByRole("button", { name: "Doku entfernen" })
    .dispatchEvent("click");
  ...
  // Clean up the space.
  await page
    .getByRole("button", { name: "Page Del entfernen" })
    .dispatchEvent("click");
  await page.getByRole("button", { name: "Löschen" }).dispatchEvent("click");
