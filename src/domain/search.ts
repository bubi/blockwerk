 ...
 * case-insensitive substring. Blocks come first, newest date first like the
 * page stream; items follow ordered by their block's date, then stream
 * position. Both lists are capped at SEARCH_LIMIT. The worker and the client
 * both go through this function — nothing re-derives the rule in SQL.
 */
export function searchMatches(blocks: readonly BlockRow[], items: readonly ItemRow[], rawQuery: string): SearchMatch {
  ...
  const hitBlocks = blocks
    .filter((block) => block.title.toLowerCase().includes(q))
