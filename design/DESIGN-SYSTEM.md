...
Zwei dokumentierte Ausnahmen: die Rautenmarke vor Überschriften läuft mit −0.05em, damit `##` als ein Zeichen liest; die **Wortmarke** mit 17 px/600 und −0.022em (`--ls-wordmark`), weil bei normaler Weite eine Wortmarke in Einzelbuchstaben zerfällt. Der Wert steht als Token, nicht gestreut.
...
Zwei Ausprägungen. **Sichtbar** — Fläche `--side` oder `--white`, Rahmen 1 px `--line`, Radius `--r-sm`, Innenabstand `--s2` `--s4`. Bei Fokus: Fläche weiß, Rahmen `--line`, dazu `0 0 0 3px var(--green-soft)`. Die Eingabezeile am Fuß der Blockkarte (`.comp`) ist ein sichtbares Feld in dieser Ausprägung — sie liegt nicht im Textstrom, sondern gehört zur Karte. **Unsichtbar** — Notiz-, Aufgaben-, Termin-, Überschrift- und Titelzeilen tragen keinen Rahmen und keine Fläche; sie sehen aus wie Text und verhalten sich wie Text. Das ist der Normalfall im Blockstrom.
...
| **Bereichsleiste** (`.rail`)      | Fläche `--side`. Oben Sprung auf „Heute" mit Zähler überfälliger Aufgaben, darunter Gruppen „Personen" und „Themen", unten angeheftet Templates und Identität. Zeilen 8/12 innen, Plakette 28 × 28. **Zweite Ebene:** Unter dem aktiven Bereich (Akkordeon, ein zweiter Klick klappt zu) stehen seine Einträge als Unterliste — Person: **Aufgaben**, **Jour Fix**, dann die Notizseiten; Thema: **Jour Fix**, dann die Notizseiten. Untereinträge eingerückt (`--s5`), kleiner gewichtet (14 px) als der Bereichsname, aktiver Eintrag wie üblich (`--green-soft` / `--green-dk`), Symbole (Seite umbenennen ✎, entfernen ×, „+ Seite") erst bei Hover/Fokus der Zeile. Der Zähler offener Aufgaben einer Person steht am Eintrag „Aufgaben", solange aufgeklappt, sonst an der Bereichszeile. „Aufgaben" und „Jour Fix" sind virtuelle Einträge: nicht umbenennbar, nicht löschbar.                                                                                                                                                                            |
...
| **Marke** (`.mark`)               | Inline-SVG im 64er-Raster (eine Blockkarte: Datumsrücken, zwei Notizzeilen, ein erledigter Task; der Haken ist das einzige Schrägelement). Parameter Größe und Tonlage (`color`, `ink`, `muted`), Farben über CSS-Variablen. `aria-hidden` — daneben steht immer die Wortmarke oder ein Seitentitel. Wortmarke: 17/600 mit `--ls-wordmark` (Ladeansicht 22 px, mobile Startseite 14 px in `--text3`). Steht an genau drei Stellen: Desktop-Kopfzeile, Ladeansicht, mobile Startseite „Heute".                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| 3   | `font-size`                                            | nur 11, 12, 13, 14, 15, 16, 18, 22, 28, 36, 44, die Glyphengrößen 16, 20, 24, 28 sowie die Wortmarke 17 px                |
| 4   | `letter-spacing` negativ                               | nur −.02, −.03, −.04 sowie die dokumentierten Ausnahmen −.05 (Rautenmarke) und −.022em (Wortmarke, Token `--ls-wordmark`) |
