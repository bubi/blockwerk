...
export async function createPage(db: D1Database, input: NewPageInput, now: number): Promise<PageRow> {
  await db
...
export async function getPage(db: D1Database, id: string): Promise<PageRow | null> {
...
export async function updatePage(db: D1Database, id: string, patch: PagePatch, now: number): Promise<PageRow | null> {
