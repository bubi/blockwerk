...
async function insertItemRow(db: D1Database, row: ItemRow): Promise<void> {
  await db
    .prepare(
      `INSERT INTO items
        (id, block_id, kind, position, text, heading, done, due_date, assignee_space_id, event_date, event_time, ref_block_id, parent_item_id, list_mark, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    )
    .bind(
      row.id,
...
export async function updateItem(
  ...
  if (patch.listMark !== undefined) set("list_mark", patch.listMark);
  if (patch.done !== undefined) set("done", patch.done ? 1 : 0);
  if (patch.dueDate !== undefined) set("due_date", patch.dueDate);
  if (patch.assigneeSpaceId !== undefined)
    set("assignee_space_id", patch.assigneeSpaceId);
...
export async function listBlockItems(
  db: D1Database,
  blockId: string,
): Promise<ItemRow[]> {
  const { results } = await db
    .prepare("SELECT * FROM items WHERE block_id = ?")
    .bind(blockId)
    .all<RawItemRow>();
 ...
 */
export async function createItemWithRespace(
  db: D1Database,
  input: NewItemInput,
  now: number,
  ...
  ordered.forEach((item, positionIndex) => {
    const position =
      1000 * (positionIndex < index ? positionIndex + 1 : positionIndex + 2);
